package com.hail.customdemo.utils;
...
public class AnalogClock extends View {
    ...
    //用来记录View是否被加入到了Window中，我们在View attached到 //Window时注册监听器，
    // 监听时间的变更，并根据时间的变更，改变自己 //的绘制，在View从Window中剥离时，解除注册，
    // 因为我们不需要再监听 //时间变更了，没人能看得到我们的View了。
...
//时要进行适当的缩放。
    ...
    public AnalogClock(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
        ...
        if (mDial == null) {
            mDial = resources.getDrawable(R.drawable.clock_hand_dial);
        }
        if (mHourHand == null) {
    ...
    protected void onAttachedToWindow() {
        ...
        if (!mAttached) {//用来记录View是否被加入到了Window中
            mAttached = true;
    ...
    protected void onDetachedFromWindow() {
        ...
        if (mAttached) {
            getContext().unregisterReceiver(mIntentReceiver);//解除注册
            mAttached = false;
        }
    }
...
/* 请注意，这里的availableWidth和availableHeight，
 每次绘制时是可能变化的， 我们可以从mChanged变量的值判断它是否发生了变化，
  如果变化了，说明View的尺寸发生了变化， 那么就需要重新为时针、分针设置Bounds，
   因为我们需要时针，分针始终在View的中心。*/

        int availableWidth = super.getRight() - super.getLeft();
        int availableHeight = super.getBottom() - super.getTop();
/* 这里的x和y就是View的中心点的坐标， 注意这个坐标是以View的左上角为0点，
向右x，向下y的坐标系来计算的。 这个坐标系主要是用来为View中的每一个Drawable确定位置。
就像View的坐标是用parent的左上角为0点的坐标系计算得来的一样。
 简单来讲，就是ViewGroup用自己左上角为0点的坐标系为 各个子View安排位置，
 View同样用自己左上角为0点的坐标系 为它里面的Drawable安排位置。 注意不要搞混了。*/
...
这里的Bounds就相当于是为Drawable在View中确定位置，
 只是确定的方式更直接，直接在View中框出一个与Drawable大小 相同的矩形，
  Drawable就在这个矩形里绘制自己。 这里框出的矩形，是以(x,y)为中心的，
  宽高等于表盘图片的宽高的一个矩形， 不用担心表盘图片太大绘制不完整，
  因为我们已经提前进行了缩放了。*/
...
/*根据小时数，以点(x,y)为中心旋转坐标系。
        ...
        if (changed) {
            ...
            /* 仔细体会这里设置的Bounds，我们所画出的矩形，
             同样是以(x,y)为中心的 矩形，时针图片放入该矩形后，
             时针的根部刚好在点(x,y)处， 因为我们之前做时针图片时，
...
/*这里我们为什么不直接把minute设置给mMinutes，而是要加上
 second /60.0f呢，这个值不是应该一直为0吗？ 这里又涉及到Calendar的 一个知识点，
  也就是它可以是Linient模式， 此模式下，second和minute是可能超过60和24的，
  具体这里就不展开了， 如果不是很清楚，建议看看Google的官方文档中讲Calendar的部分*/
